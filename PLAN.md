...
owner: "@moberghr/lending"  # who drift routes this page to; carried verbatim, never resolved (§6.4)
...
```jsonc
{
  ...
  "pages": {
    "10-domains/loans/README.md": {
      ...
      "verdict": {                          // sealed by publish: the source bytes the live body was generated from (§6.4)
        "sourcesHash": "sha256:…",          // fingerprint of the files this page's `sources` globs matched
        "sealedAt": "2026-08-19T09:12:44Z", // when the seal was taken
        "repoSha": "6accfb8…"               // the commit the sealing run published; the audit trail for a dirty tree
      },
      "approval": {
...
### 6.3 `docume sync [--labels] [--comments] [--output-dir <path>] [--rebuild-state]`
Default: both.
...
- **Rebuild** (`--rebuild-state`, runs alone; composes only with `--dry-run`): lists every page in the space and reads the `docume` managed marker §6.2 stamps, rebuilding state's page map from the pages' own testimony. It is the recovery path for a lost, forked or corrupted `state.json` (docs/specs/2026-08-19-state-rebuild.md). Each stamped page lands in a manifest as **adopted** (path names a file this repo has and state makes no claim: an entry is written with pageId, title, `marked` and nothing else), **already tracked**, **conflicted** (state names a different id, or two pages stamp the same path; nothing written, the manifest says what disagrees), or **path-missing** (no such file: left out of state, which is the safe side, since prune cannot touch what state does not hold). The file check is also what keeps two repos sharing one space apart: each adopts only the paths it holds, in practice though not by proof, since the marker carries no tenant field. Approvals, hashes and staleness are never rebuilt: approvals belong to reviewers and hashes to the next publish, which updates each adopted page and re-records them honestly. The walk is reads plus one state-file write, so the §1.4 space write-lock has nothing to refuse in it.
...
A third narrowing is **computed rather than declared**, and it is the only one nobody has to write down: each publish seals a fingerprint of the files a page's `sources` globs matched into that page's `verdict` (§5.3), taken over the files git tracks so build output can never be in it, and `drift` recomputes it for the flagged pages only. A page whose sources are byte-identical to its seal is reported as **sealed** rather than drifted, which is what stops a revert inside the range, a merge that re-introduces identical bytes, and a `baselineSha` older than the page's own last publish from producing phantom drift. Sealed pages are disclosed in every format (the table's `SEALED` block with the date of each seal, the JSON's `sealed`, the PR comment's own section), count toward neither the exit code nor `--mark`, and are held out only on proof: a page with no seal, and a page whose sources cannot be read now, both keep the range-based answer, because a fingerprint nobody could compute must never suppress a report. A seal is always evidence about at least one file — a page whose globs match no tracked file, and a run whose tracked-file list is empty, seal nothing and say so, and `drift` refuses the fingerprint of no files from the other side too, since that one value is what every such condition produces and every later run under it reproduces. The claim stays narrow: these were the source bytes when the live body was published. It is not an approval (§8 is untouched), it says nothing about whether the prose is right, and it is only as honest as the publish that wrote it; a publish from a dirty working tree seals uncommitted bytes, which is what `repoSha` exists to make auditable afterwards.

A report that names the right pages is still addressed to nobody, so a page may name its owner. **`owner:` in frontmatter** (§5.2) is a single string carried verbatim into the report: the PR comment groups the affected pages under one `**Owner:**` heading per owner, ordinal by the owner string so the comment a bot rewrites in place on every push produces the same order every time, and a handle spelled the way that forge mentions people becomes a real notification rather than a line of text. Pages that declare none group last under `**No owner**`, whose heading states how many they are and says the drift is addressed to nobody; the table's verdict line discloses the same count, and `--format json` carries `owner` per page plus `unownedCount`, with an unowned page carrying no `owner` key at all so "unowned" has one spelling on the wire. DocuMe never prepends `@`, never folds case and never resolves the value: what a mention looks like on a given forge is the consumer repo's knowledge (rule §9.5), and turning `alice` into `@alice` would notify whichever account happens to hold that name. Verbatim stops where a forge handle stops, neutralized at the renderer rather than in the parser: the PR comment collapses line endings and neutralizes `<`, `[` and `]`, because a YAML scalar carries a newline two ways, inline raw HTML needs none, and `[label](url)` needs neither, so each of the three lets a crafted `owner` forge a verdict, hide the report behind a `<details>`, or post a link to anywhere inside a comment the bot signs; no forge handle carries any of them, which is the test that decides the set, and `_` and `*` are left alone for exactly that reason, so the mention is unaffected, and the other untrusted strings this comment renders (page and seal titles, seal dates, exemption reasons, both revisions) are line-collapsed and markdown-escaped together by one helper. The grouping partitions the affected list rather than filtering it, so every affected page appears exactly once and nothing already held out is routed: sealed pages and exempted files never enter the list the grouping reads.
